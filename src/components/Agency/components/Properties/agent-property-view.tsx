...
const AgentPropertyDetailPage = () => {
  ...
  const handleMarkAsTaken = async () => {
    await axiosInstance
      .delete(`/property/archive-property/${id}`)
      .then((response) => {
      ...
      .catch((err) => {
        ...
        toast.error("Action Failed", {
          description: "Unable to update property status. Please try again.",
        });
      });
  };
  ...
  useEffect(() => {
    const boostPaymentReference = localStorage.getItem(
      "boost_payment_reference"
    );
    if (boostPaymentReference) {
      // verify payment status from backend
      axiosInstance
        .get(`/payment/boost/verify/${boostPaymentReference}`)
        .then((response) => {
          if (response?.data?.success) {
            toast.success("Property Boosted!", {
              description:
    ...
    <div className="flex flex-col min-h-screen">
      <main className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => router.push("/agent-dashboard/properties")}>
            Back to Listings
          </Button>
        </div>
      </main>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <div className="flex flex-row items-center gap-4 mt-4">
              {getStatusBadge(
                !propertyData?.isPropertyTaken ? "Active" : "Taken"
              )}
          ...
          <div className="text-right">
            ...
            <Badge variant="outline" className="mt-1 capitalize">
              {propertyData?.propertyCategory?.toLowerCase()}
            </Badge>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              <CardContent className="p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-semibold">
                        {propertyData?.sizeInSquareFeet?.toLocaleString()}
                      </div>
                      <div className="text-sm text-muted-foreground">
                        Sq&nbsp;Ft
                      </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div className="">
                    ...
                    <p className="font-medium">
                      {propertyData?.geoCoordinates?.latitude}
                    </p>
                  </div>
                  <div className="">
                    <span className="text-muted-foreground">Longitude:</span>
                    <p className="font-medium">
                      {propertyData?.geoCoordinates?.longitude}
                    </p>
                  </div>
                  <div className="">
                    <span className="text-muted-foreground">Listed:</span>
                    <p className="font-medium">
                      {new Date(propertyData?.dateCreated).toLocaleDateString()}
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem
                    onClick={() =>
                      handleShare("facebook", propertyData?.video?.url)
                    }
                  >
                  ...
                  <DropdownMenuItem
                    onClick={() =>
                      handleShare("twitter", propertyData?.video?.url)
                    }
                  >
                  ...
                  <DropdownMenuItem
                    onClick={() =>
                      handleShare("whatsapp", propertyData?.video?.url)
                    }
                  >
                  ...
                  <DropdownMenuItem
                    onClick={() =>
                      handleShare("copy", propertyData?.video?.url)
                    }
                  >
            ...
            {/* architectural plans */}
            {propertyData?.propertyArchPlans &&
              propertyData?.propertyArchPlans?.length > 0 && (
                <Card>
            ...
            <Card>
              <CardContent className="p-6 space-y-4">
                ...
                <Button
                  ...
                  variant={
                    propertyData?.isPropertyTaken ? "default" : "destructive"
                  }
                  className="w-full"
...
const ConfirmMarkAsTakenPrompt = ({
  isOpen,
  onClose,
  onConfirm,
}: {
  ...
  return (
    <div className="fixed inset-0 bg-white/60 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-11/12 max-w-md">
        ...
        <p className="mb-6">
          Are you sure you want to mark this property as taken? This action
          cannot be undone.
        </p>
...
export const BoostPropertyPrompt = ({
  isOpen,
  onClose,
  id,
  setBoost,
  ...
  const onChangeWeeks = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (weeks < 1) {
      toast.error("Invalid Input", {
        description: "Number of weeks must be at least 1.",
      });
    }
  ...
  const handleBoost = async () => {
    await axiosInstance
      ...
      .catch((err) => {
        ...
        toast.error("Action Failed", {
          description:
            "Unable to complete the Boosting Process. Please try again.",
        });
      });
  };
      ...
      <Card>
        <CardContent>
          <div className="bg-white rounded-lg p-6 w-12/12 max-w-md">
            ...
            <p className="mb-6">
              Boosting your property will increase its visibility in search
            ...
            <Input
              type="number"
              min={1}
              placeholder="Enter number of weeks to boost"
              className="mb-6"
