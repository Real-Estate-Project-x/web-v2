...
const Viewings = () => {
  ...
  useEffect(() => {
    async function getViewings(){
      try{
        const viewingsResponse = await axiosInstance.get(`/agent-property-viewing/agency/viewing-list/${agencyId}?filter=${statusFilter}`);
  ...
  const upcomingViewings = viewings?.filter(v => v?.viewingStatus === "UPCOMING").length;
  const pendingViewings = viewings?.filter(v => v?.viewingStatus === "PENDING").length;
  const todayViewings = viewings?.filter(v => v?.viewingStatus === "TODAY").length;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Viewings Today</CardTitle>
              <Eye className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{todayViewings ? todayViewings : 0}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="flex items-center gap-2">
                <Select value={sortBy} onValueChange={setSortBy}>
                  ...
                  <SelectContent>
                    <SelectItem value="VIRTUAL">Virtual&nbsp;Viewings</SelectItem>
                    <SelectItem value="IN_PERSON">In-person&nbsp;Viewings</SelectItem>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <Badge className={viewing?.medium === "IN_PERSON" ? "bg-blue-900 text-white p-2 mb-2" : "bg-slate-300 text-black p-2 mb-2"}>{viewing?.medium}&nbsp;VIEWING</Badge>

                          <h3 className="text-lg font-semibold mb-1 capitalize">{viewing?.property?.title}</h3>
                          ...
                          <div className="flex items-start gap-4 text-sm text-muted-foreground">
                            ...
                            <p className="flex flex-col space-y-2">
                              <span className="flex items-center gap-1 text-base">
                                ...
                                {format(reformatDate(viewing?.availabilityWindow?.date), "EEEE, MMMM d, yyyy")}
                              </span>
                        ...
                        <Badge className={`${getStatusColor(viewing?.viewingStatus)} p-2`}>
                          {viewing?.viewingStatus.charAt(0).toUpperCase() + viewing?.viewingStatus.slice(1)}
                        </Badge>
                      </div>
                      {viewing?.price &&
                      ...
                      {viewing?.propertyId && 
                        <Link href={`/agent-dashboard/properties/view?id=${viewing?.propertyId}`} 
                        target="_blank" 
                    ...
                    <div className="w-fit">
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div className="flex-1">
                          <h4 className="font-medium">{viewing?.user?.firstName}&nbsp;{viewing?.user?.lastName}</h4>
                      ...
                      <div className="mb-3 pl-2">
                        {(viewing?.medium === "VIRTUAL" && !viewing?.isPaid)&&
                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                              ...
                              <Link href={viewing?.meetingLink ?? ""} target="_blank" className="text-sm text-blue-600 italic">Click to Join</Link>  
                            </div>
                        }
                      </div>
