  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLocalStorageField(
        "agentId",
        userProfileResponse?.data?.data?.agenciesCreated?.[0]?.id
      );
    };
    fetchData();
  }, []);
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
            <Avatar className="h-24 w-24 bg-gray-200 rounded-full flex items-center justify-center mx-auto">
              <AvatarImage
                src={agentData?.profileImage?.image?.url}
                alt={agentData?.agenciesCreated?.[0].name}
              />
              <AvatarFallback>
                <User />
              </AvatarFallback>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="mx-auto md:mx-0">
                  <h1 className="text-3xl font-bold uppercase text-center md:text-start">
                    {agentData?.agenciesCreated?.[0].name}
                  </h1>
                  <p className="text-lg text-muted-foreground capitalize text-center md:text-start">
                    {agentData?.agenciesCreated?.[0]?.description}
                  </p>
                  <div className="flex items-center gap-4 mt-2">
                    <div className="flex items-center gap-1 mx-auto md:mx-0">
                      <Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                      <span className="font-medium">
                        {agentData?.agenciesCreated?.[0]?.rating}
                      </span>
                    </div>
                  </div>
                </div>
