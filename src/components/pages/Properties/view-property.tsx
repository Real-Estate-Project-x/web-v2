...
import { CommentInterface, PropertyInterface, ViewPropertyInterface } from "../../../../utils/interfaces";
...
const PropertyDetails = () => {
  ...
  const [propertyData, setProperty] = useState<ViewPropertyInterface>({} as ViewPropertyInterface);
  ...
  useEffect(() => { 
   ...
   const ratingConstant = propertyData?.property?.bluepoddRating ? propertyData.property.bluepoddRating : 0;
              ...
              <div className="container mx-auto px-4 py-8 mt-16">
                <div className="mb-8">
                  <div className="flex justify-between items-start mb-2">
                    <h1 className="text-2xl md:text-3xl font-semibold text-navy-900">{propertyData.property?.title || ""}</h1>
                    <div className="flex items-center">
                      <div className="flex mr-2">
                        {renderRating(ratingConstant)}
                        <span className="ml-2 text-gray-600">({ratingConstant})</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center text-gray-600 mb-4">
                    <MapPin className="h-5 w-5 mr-2" />
                    <p className="text-base capitalize">{new String(propertyData?.property?.address ? propertyData.property.address : '').toLowerCase()}</p>
                  </div>
                ...
                <div className="grid grid-cols-1 gap-8">
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    <div className="grid grid-cols-5 gap-4 py-4 border-y">
                      <div className="text-center">
                        <div className="flex items-center justify-center">
                          ...
                          <span className="text-lg font-semibold">{propertyData?.property?.noOfBedrooms}</span>
                        </div>
                        <p className="text-sm text-gray-600">Bedrooms</p>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-semibold">{propertyData?.property?.noOfToilets}</div>
                        <p className="text-sm text-gray-600">Bath/Toilet(s)</p>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-semibold">{propertyData?.property?.sizeInSquareFeet}</div>
                        <p className="text-sm text-gray-600">Square Feet</p>
                      </div>
                      {propertyData?.property?.additionalCosts && propertyData.property.additionalCosts.length > 0
                        ? propertyData.property.additionalCosts.map((cost:{amount : number, title : string}, index) => (
                            <div className="text-center" key={index}>
                              <div className="text-lg font-semibold">{formatPrice(cost.amount || 0)}</div>
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 leading-relaxed text-sm md:text-base capitalize">{propertyData?.property?.description}</p>
                    </div>
                ...
                <div className="mt-16">
                  ...
                  <div className="relative overflow-hidden">
                    <Carousel className="w-full">
                      <CarouselContent>
                        {propertyData.similarProperties?.slice(0, 10).map((similarProperty : Partial<PropertyInterface>) => (
                          <CarouselItem key={similarProperty.id} className="md:basis-1/1 lg:basis-1/2">
                            <Card className="overflow-hidden flex items-center gap-2 flex-row hover:shadow-lg transition-shadow h-full py-0">
                              <div className="w-1/2 relative h-56">
                                <img
                                  ...
                                  className="w-full h-full object-cover"
                                />
                                <Badge className="absolute top-4 right-4 bg-white text-[#102A43] p-2 rounded-full">{formatPrice(similarProperty?.price ? similarProperty.price : 0)}</Badge>
                                <Badge className={`${similarProperty?.upFor?.toUpperCase() === "RENT" ? "bg-[#0253CC]" : "bg-green-800"} absolute top-4 left-4 capitalize px-4 py-2 font-normal rounded-full`}>{new String(similarProperty?.upFor).toLowerCase()}</Badge>
                                <Badge className="absolute bottom-4 right-4 bg-white text-[#102A43] cursor-pointer h-8 w-8"onClick={() => window.location.href = `/properties/view/?id=${similarProperty.id}`}><Eye className="w-20 h-20"/></Badge>
                              </div>
                              <CardContent className="w-1/2 p-4">
                                <h3 className="text-lg font-semibold mb-2 capitalize">{similarProperty?.title}</h3>
                                <div className="flex items-center text-gray-600 mb-3">
                                  <MapPin className="h-4 w-4 mr-1" />
                                  <span className="text-sm capitalize">{similarProperty?.address}</span>
                                </div>
                                <div className="flex justify-between text-gray-600 border-t pt-3">
                                  <div className="flex items-center">
                                    <Bed className="h-4 w-4 mr-1" />
                                    <span className="text-sm">{similarProperty?.noOfBedrooms} beds</span>
                                  </div>
                                  <div className="flex items-center">
                                    <span className="text-sm">{similarProperty?.noOfToilets} baths</span>
                                  </div>
                                  <div className="flex items-center">
                                    <span className="text-sm">{similarProperty?.sizeInSquareFeet} sqft</span>
                                  </div>
                                </div>
                                {/* <Button 
