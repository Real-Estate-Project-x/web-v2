...
const VPN_APPS = [
  ...
  { name: "Warp by Cloudflare", url: "https://one.one.one.one" },
];
...
export default function RegionNotSupported() {
  ...
  const fetchSupportedCountries = async () => {
    const url =
      "/country/?isSignupEnabled=true&fields=success,data(id,code,flag,name)";

    try {
      const result = await axiosInstance.get(url);
      if (result.data?.success) {
        setSupportedCountries(result.data.data);
      }
    } catch (ex) {
      throw ex;
    }
  ...
  return (
    <>
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-6 py-16 font-sans">
        ...
        <div className="bg-white border border-gray-200 rounded-3xl px-8 py-10 max-w-md w-full text-center shadow-sm">
          ...
          <p className="text-sm text-gray-400 leading-relaxed mb-8">
            Blupodd isn't available in your country just yet. We're expanding
            fast — but for now, the platform is only accessible in a handful of
            supported regions.
          </p>
          ...
          <div className="bg-slate-50 rounded-2xl px-5 py-4 mb-6 text-left">
            ...
            <div className="flex flex-col gap-2">
              {supportedCountries.map((r) => (
                <Link href={`/properties/by-country/?name=${r.code}`}>
                  <div
                    key={r.id}
                    className="cursor-pointer flex items-center gap-2.5"
                  >
                    <span className="text-base">
                      <ReactCountryFlag countryCode={r.code} svg />
                    </span>
            ...
            {showVpnTip && (
              <div className="mt-4 pt-4 border-t border-gray-100">
                <p className="text-xs text-gray-400 leading-relaxed mb-3">
                  You can use a VPN to connect from a supported region. Set your
                  server location to one of the countries above, then reload
                  Blupodd.
                </p>
      ...
      {modalIsOpen && <JoinWaitlist setIsModalOpen={setModalIsOpen} />}
    </>
  );
}
