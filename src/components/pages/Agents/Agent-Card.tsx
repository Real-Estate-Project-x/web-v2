...
const AgentCard = ({ agent: data }: AgentCardProps) => {
  ...
  return (
    ...
    <div className="relative group h-full">
      ...
      <div className="relative h-full bg-[#0f1c36] border border-blue-700/30 rounded-2xl overflow-hidden transition-all duration-300 group-hover:border-blue-500/50"
        ...
        <div className="relative p-6 flex flex-col h-full">
          ...
          <div className="flex items-start gap-4 mb-5">
            <div className="relative shrink-0">
              {/* Avatar glow */}
              <div className={`absolute inset-0 rounded-full blur-md opacity-40 scale-110 ${agent?.status ? 'bg-sky-400' : 'bg-blue-900'}`} />
              ...
              <div
                className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-[#0f1c36] shadow-lg ${agent?.status ? "bg-sky-400 shadow-sky-400/50" : "bg-blue-900"
                  }`}
              />
            </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2 mb-0.5">
                ...
                <span
                  className={`shrink-0 inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full border ${data?.isCurrentlyAvailable
                    ? "bg-sky-400/10 text-sky-300 border-sky-400/20"
                    : "bg-blue-900/30 text-blue-400 border-blue-800/40"
                    }`}
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${data?.isCurrentlyAvailable ? 'bg-sky-400' : 'bg-blue-600'}`} />
                  {data?.isCurrentlyAvailable ? "Available" : "Unavailable"}
                </span>
              </div>

              <p className="text-xs text-white/40 mb-2 leading-relaxed" style={{ overflowWrap: "break-word" }}>
                {agent?.description}
              </p>

              <div className="flex items-center gap-1">
                <Star className="h-3 w-3 fill-sky-400 text-sky-400" />
                <span className="text-xs font-bold text-sky-400">{agent?.rating}</span>
                <span className="text-xs text-white/25">({data?.totalReviewCount} reviews)</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-5">
            {[
              ...
              <div key={i} className="text-center bg-blue-900/30 border border-blue-700/30 rounded-xl py-2.5 px-1">
                <div className="text-sm font-bold text-white capitalize leading-none mb-1">{stat.value}</div>
                <div className="text-[10px] text-white/30 leading-none">{stat.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="text-xs text-white/40 capitalize" style={{ overflowWrap: "break-word" }}>
              {agent?.address}
            </p>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-white/50">{agent?.whatsappNumber}</span>
            </div>

            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 min-w-0">
                <Mail className="h-3.5 w-3.5 text-white/30 shrink-0" />
                <span className="text-xs text-white/50 truncate">{agent?.email}</span>
              </div>
