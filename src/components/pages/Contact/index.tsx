...
const ContactUsComponent = () => {
  ...
  const [success, setSuccessObj] = useState<{ msg: string; flag: boolean }>({
    msg: "",
    flag: false,
  });
        ...
        <div className="bg-white/90 rounded-lg shadow-lg max-w-2xl w-full p-8 mb-10 mt-16 relative">
          ...
          <form className="space-y-4" ref={formRef} onSubmit={submitHandler}>
            <div>
              <label
                htmlFor="name"
                className="block text-navy-800 mb-1 font-medium"
              >
                Name
              </label>
              <Input
            ...
            <div>
              ...
              <Input
                ...
                name="email"
                type="tel"
                placeholder={"07097230988"}
                defaultValue={phone}
                required
                maxLength={11}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  onChangePhoneNumberHandler(e.target.value)
                }
              />
            </div>
            <div>
            ...
            <Button
              className="bg-[#1D4ED8] hover:bg-[#2563EB] text-white w-full sm:w-1/2 mx-auto mt-2 float-right disabled:cursor-default disabled:bg-gray-500"
              type="submit"
              disabled={loading}
            >
              Send Message
